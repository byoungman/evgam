...
#' @param bgev.args a list of arguments for \code{family="bgev"}; see Details
...
#' \code{evgam} using \code{family = "..."} and details of \code{gpd.args}, \code{ald.args},  
#' \code{exi.args}, \code{pp.args}, \code{bgev.args} and \code{egpd.args} and see 
...
simulate.evgam <- function(object, nsim=1e3, seed=NULL, newdata, 
  ...
  if (type == "quantile" & is.null(probs)) 
    stop("non-NULL `prob' required if `type = quantile'")
    
  if (is.null(q_fn) & type == 'quantile' & family != 'custom')
    stop(paste("type = 'quantile' not currently supported for family = '", family, "'", sep = ''))
    
               
  if (!is.null(probs)) 
    type <- "quantile"
  # if (is.null(newdata)) newdata <- object$data
  if (type %in% c("link", "response", "quantile")) {
  ...
  if (type == "quantile") {
    ...
    out0 <- X
    names(out0) <- paste('pars', seq_along(out0), sep = '')
    out <- list()
    
    for (i in seq_along(probs)) {
      
      out0$p <- probs[i]
      out[[i]] <- do.call(q_fn, out0)
